...
from LanguageSelector.ABRootTransaction import ABRootTransaction
from LanguageSelector.RunAsync import RunAsync
...
#            self.updateSystemDefaultCombo()
    ...
    def _commit_aptdaemon(self, inst, rm):
        # This is a workaround, since Vanilla OS does not allow
        # direct transactions, but only via ABRoot, so we cannot
        # use the aptdaemon directly. This will be replaced by
        # a proper solution in the future, probably a whole new
        # application for the Vanilla OS.

        def callback(result, *args):
            if result:
                inst_dialog.destroy()
                return subprocess.run(['gnome-session-quit', '--reboot'])
            ...
            dialog = Gtk.MessageDialog(
                ...
                text=_('Failed to install packages. Please try again later.'))
            dialog.run()
            dialog.destroy()
        ...
        if len(inst) == 0 and len(rm) == 0:
            return
        try:
            yield RunAsync(self.ab.commit_packages, callback, inst, rm)
            inst_dialog = Gtk.MessageDialog(
                transient_for=self.window_main,
                modal=True,
                message_type=Gtk.MessageType.INFO,
                buttons=Gtk.ButtonsType.NONE,
                text=_('Installing packages...'))
            inst_dialog.format_secondary_text(
                _('Please wait while the packages are being installed.'))
            inst_dialog.show_all()
            inst_dialog.set_keep_above(True)
        except Exception as e:
            self._show_error_dialog(e)
